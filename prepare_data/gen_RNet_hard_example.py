...
def save_hard_example(gen_anno_file, gen_imgs_dir,data,save_path,test_mode):
    ...
    for im_idx, dets, gts in zip(im_idx_list, det_boxes, gt_boxes_list):
        ...
        for box in dets:
            ...
            if  x_left < 0 or y_top < 0 or x_right > img.shape[1] - 1 or y_bottom > img.shape[0] - 1 or width <=2 or height<=2:
                #print("pass")
                cnt_pass+=1
                continue
            ...
            union_max = np.max(Iou) 
            gt_max = np.max(Iou_gt)        
            if union_max <=0.2 and neg_num < 60:
                #save the examples
                idx = np.argmax(Iou)
                assigned_gt = gts[idx]
                ...
                #if np.max(Iou) >= 0.4:
                #if np.max(Iou) >= 0.65:
                if union_max >= 0.6 and offset_y1>Y1_thresh and np.abs(offset_x1)<= X1_thresh:
                    ...
                    #elif np.max(Iou) >= 0.3:
                #elif np.max(Iou) >= 0.4:
                elif union_max <= 0.4 and union_max >0.1 and offset_y1 <=Y1_thresh:
...
def rd_anotation(img_saved_dir,filename):
    data = dict()
    image_path_list = []
    boxes_gd = []
...
def t_net(prefix, epoch,batch_size, img_saved_dir,anno_file,gen_anno_file,gen_imgs_dir,ignore_det=False,test_mode="PNet",thresh=[0.6, 0.6, 0.7], min_face_size=25,\
             stride=2):
    slide_window=False
    ...
    img_box_dic = rd_anotation(img_saved_dir,anno_file)
...
def parse_args():
    ...
    parser.add_argument('--prefix', dest='prefix', help='prefix of model name', nargs="+",
                        default=['../data/MTCNN_caltech_model/PNet_landmark/PNet', '../data/MTCNN_caltech_model/RNet_landmark/RNet', '../data/MTCNN_caltech_model/ONet_landmark/ONet'],
                        type=str)
    parser.add_argument('--epoch', dest='epoch', help='epoch number of model to load', nargs="+",
                        default=[1600, 2900, 22], type=int)
    parser.add_argument('--batch_size', dest='batch_size', help='list of batch size used in prediction', nargs="+",
                        default=[1, 2048, 16], type=int)
    parser.add_argument('--thresh', dest='thresh', help='list of thresh for pnet, rnet, onet', nargs="+",
                        default=[0.6, 0.6, 0.7], type=float)
    parser.add_argument('--min_face', dest='min_face', help='minimum face size for detection',
                        default=24, type=int)
    parser.add_argument('--stride', dest='stride', help='stride of sliding window',
                        default=2, type=int)
    parser.add_argument('--anno_file',type=str,default="./wider_face_train_bbx_gt.txt",\
...
if __name__ == '__main__':
    ...
    t_net(prefix, epoch_list,batch_size, img_saved_dir,anno_file,gen_anno_file,gen_imgs_dir,ignore_det,test_mode,score_thresh, min_face_size,stride)
